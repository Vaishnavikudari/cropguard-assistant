...
def google_login():
    ...
    if row:
        ...
        conn.close()
        return jsonify({"success": True, "user": {"id": user_id, "name": existing_name}})
    else:
        dummy_hash = generate_password_hash(f"google_{google_id}")
        c.execute("INSERT INTO users (name, phone, email, state, crop, password_hash) VALUES (?, ?, ?, ?, ?, ?)",
                  (name, None, email, None, None, dummy_hash))
        conn.commit()
        user_id = c.lastrowid
        conn.close()
        return jsonify({"success": True, "user": {"id": user_id, "name": name}})
